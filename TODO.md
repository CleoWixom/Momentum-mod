...
| RSSI-01 | ⬜ Открыто | RSSI | 🟠 Высокий | Средняя | Неточное время чтения RSSI (после AGC settle) |
| RSSI-02 | ⬜ Открыто | RSSI | 🟡 Средний | Низкая | Нет усреднения — показания шумят ±5 дБм |
| RSSI-03 | ⬜ Открыто | RSSI | 🟡 Средний | Средняя | AGC мешает точным измерениям в Freq Analyzer |
| RSSI-04 | ⬜ Открыто | RSSI | 🟡 Средний | Средняя | RSSI не сохраняется в истории сигналов |
...
**Что сделать:**
- [ ] Читать RSSI в середине пакета: по событию `GDO0` (sync word detected) → задержка ~половина символов → чтение регистра.
- [ ] Альтернатива: сохранять RSSI при событии `GDO2` (carrier sense) — до того как AGC успевает сильно адаптироваться.
- [ ] Добавить поле `rssi_at_sync` в структуру приёма и сохранять его вместе с декодированным сигналом.
...
**Что сделать:**
- [ ] Реализовать IIR-фильтр (экспоненциальное скользящее среднее) для RSSI:
  ```c
  ...
  ```
- [ ] Применить фильтр в Frequency Analyzer (отображение) и в worker (threshold-сравнение).
- [ ] Хранить `rssi_filtered` в структуре воркера, сбрасывать при смене частоты.
...
**Что сделать:**
- [ ] Для режима Frequency Analyzer (только измерение, не декодирование) — зафиксировать gain вручную через `AGCCTRL2`:
  ```c
  ...
  ```
- [ ] Восстанавливать нормальный AGC (`0xC7` default) при выходе из Frequency Analyzer.
- [ ] Добавить настройку: "точный режим RSSI" (медленнее, точнее) vs "быстрый режим" (текущее поведение).
...
**Что сделать:**
- [ ] Добавить поле `float rssi` в `SubGhzHistoryItem`.
- [ ] Заполнять при вызове `subghz_history_add_to_history()` из значения `SubGhzRadioPreset` или отдельного параметра.
- [ ] Отображать RSSI в `SubGhzViewReceiver` рядом с каждой записью истории (мелким шрифтом, дБм).
- [ ] Учитывать RSSI при дедупликации (HISTORY-03): два сигнала с одинаковым кодом, но RSSI отличается более чем на 15 дБм — разные устройства, не дедуплицировать.
